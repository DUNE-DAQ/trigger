...
#ifndef TRIGGER_INCLUDE_TRIGGER_TAWRAPPERPDS_HPP_
#define TRIGGER_INCLUDE_TRIGGER_TAWRAPPERPDS_HPP_
...
namespace trigger {
  struct TAWrapperPDS
  {
    ...
    using FrameType = TAWrapperPDS;

    triggeralgs::TriggerActivityPDS activity;
    std::vector<uint8_t> activity_overlay_buffer;
    
    // Don't really want this default ctor, but IterableQueueModel requires it
    //TAWrapperPDS() {}
    
    TAWrapperPDS(triggeralgs::TriggerActivityPDS a = triggeralgs::TriggerActivityPDS())
      : activity(a)
    {
      populate_buffer();
    }
    ...
    bool operator<(const TAWrapperPDS& other) const
    {
    ...
    TAWrapperPDS* begin()
    {
      return (TAWrapperPDS*)(activity_overlay_buffer.data());
    }
    
    TAWrapperPDS* end()
    {
      return (TAWrapperPDS*)(activity_overlay_buffer.data()+activity_overlay_buffer.size());
    }

    static const constexpr daqdataformats::SourceID::Subsystem subsystem = daqdataformats::SourceID::Subsystem::kTrigger;
    static const constexpr daqdataformats::FragmentType fragment_type = daqdataformats::FragmentType::kTriggerActivityPDS;
