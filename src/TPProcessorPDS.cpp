/**
 * @file TPProcessorPDS.hpp TPC TP specific Task based raw processor
 *
 * This is part of the DUNE DAQ , copyright 2023.
 * Licensing/copyright details are in the COPYING file that you should have
 * received with this code.
 */
#include "trigger/TPProcessorPDS.hpp" // NOLINT(build/include)
...
DUNE_DAQ_TYPESTRING(dunedaq::trigger::TriggerPrimitiveTypeAdapterPDS, "TriggerPrimitivePDS")
DUNE_DAQ_TYPESTRING(std::vector<dunedaq::trigger::TriggerPrimitiveTypeAdapterPDS>, "TriggerPrimitiveVector")

namespace dunedaq {
namespace trigger {

TPProcessorPDS::TPProcessorPDS(std::unique_ptr<datahandlinglibs::FrameErrorRegistry>& error_registry, bool post_processing_enabled)
  : TaskRawDataProcessorModel<TriggerPrimitiveTypeAdapterPDS>(error_registry, post_processing_enabled)
{
}

TPProcessorPDS::~TPProcessorPDS()
{}

void
TPProcessorPDS::start(const nlohmann::json& args)
{
...
void
TPProcessorPDS::stop(const nlohmann::json& args)
{
  inherited::stop(args);
...
void
TPProcessorPDS::conf(const appmodel::DataHandlerModule* conf)
{
  for (auto output : conf->get_outputs()) {
   try {
      if (output->get_data_type() == "TriggerActivityPDS") {
         m_ta_sink = get_iom_sender<triggeralgs::TriggerActivityPDS>(output->UID());
      }
  ...
  m_sourceid.subsystem = TriggerPrimitiveTypeAdapterPDS::subsystem;
  ...
  for (auto algo : ta_algorithms)  {
    ...
    std::shared_ptr<triggeralgs::TriggerActivityMakerPDS> maker = make_ta_maker(algo->class_name());
    nlohmann::json algo_json = algo->to_json(true);

    TLOG() << "Algo config:\n" << algo_json.dump();

    maker->configure(algo_json[algo->UID()]);
    inherited::add_postprocess_task(std::bind(&TPProcessorPDS::find_ta, this, std::placeholders::_1, maker));
...
void
TPProcessorPDS::generate_opmon_data()
{
 ...
 * */
void
TPProcessorPDS::find_ta(const TriggerPrimitiveTypeAdapterPDS* tp,  std::shared_ptr<triggeralgs::TriggerActivityMakerPDS> taa)
{
  if (m_latency_monitoring.load()) m_latency_instance.update_latency_in( tp->tp.time_start ); // time_start or time_peak ?
  m_tp_received_count++;	
  std::vector<triggeralgs::TriggerActivityPDS> tas;
...
void
TPProcessorPDS::print_opmon_stats()
{
  TLOG() << "TPProcessorPDS opmon counters summary:";
  TLOG() << "------------------------------";
