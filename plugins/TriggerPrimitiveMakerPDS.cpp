...
#include "TriggerPrimitiveMakerPDS.hpp"
...
TriggerPrimitiveMakerPDS::TriggerPrimitiveMakerPDS(const std::string& name)
  : dunedaq::appfwk::DAQModule(name)
  , m_queue_timeout(100)
{
  // clang-format off
  register_command("conf",  &TriggerPrimitiveMakerPDS::do_configure);
  register_command("start", &TriggerPrimitiveMakerPDS::do_start);
  register_command("stop_trigger_sources",  &TriggerPrimitiveMakerPDS::do_stop);
  register_command("scrap", &TriggerPrimitiveMakerPDS::do_scrap);
  // clang-format on
}

void
TriggerPrimitiveMakerPDS::init(const nlohmann::json& obj)
{
  m_init_obj = obj;
}

void
TriggerPrimitiveMakerPDS::do_configure(const nlohmann::json& obj)
{
  m_conf = obj.get<triggerprimitivemakerpds::ConfParams>();
...
void
TriggerPrimitiveMakerPDS::do_start(const nlohmann::json& args)
{
  ...
  for (auto& stream : m_tp_streams) {
    m_threads.push_back(std::make_unique<std::thread>(&TriggerPrimitiveMakerPDS::do_work,
                                                      this,
...
void
TriggerPrimitiveMakerPDS::do_stop(const nlohmann::json& /*args*/)
{
...
void
TriggerPrimitiveMakerPDS::do_scrap(const nlohmann::json& /*args*/)
{
...
std::vector<TPSet>
TriggerPrimitiveMakerPDS::read_tpsets(std::string filename, int element)
{
  TPSet tpset;
...
void
TriggerPrimitiveMakerPDS::do_work(std::atomic<bool>& running_flag,
                               std::vector<TPSet>& tpsets,
...
DEFINE_DUNE_DAQ_MODULE(dunedaq::trigger::TriggerPrimitiveMakerPDS)
