...
RandomTCMakerModule::create_candidate(dfmessages::timestamp_t timestamp)
{
  ...
  candidate.time_start = timestamp - m_tcout_time_before - m_tcout_time_after; //(timestamp - m_tcout_time_before);
  candidate.time_end = timestamp; //(timestamp + m_tcout_time_after);
  candidate.time_candidate = timestamp - m_tcout_time_after; //timestamp;
  candidate.detid = { 0 };
