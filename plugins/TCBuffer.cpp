...
TCBuffer::do_work(std::atomic<bool>& running_flag)
{
  ...
  while (running_flag.load()) {
    ...
    std::optional<triggeralgs::TriggerCandidate> tc = m_input_queue_tcs->receive(std::chrono::milliseconds(0));
    if (tc.has_value()) {  
    ...
    std::optional<dfmessages::DataRequest> data_request = m_input_queue_dr->receive(std::chrono::milliseconds(0));
