...
local info = {
   ...
   info: s.record("Info", [
       ...
       s.field("new_td_sent_count",                     self.uint8, 0, doc="Trigger decisions added to queue in the slice."),
       ...
       s.field("td_queue_timeout_expired_err_tc_count", self.uint8, 0, doc="Number of contributing trigger candidates asssociated with decisions failed to be added to queue due to timeout."),
       s.field("td_inhibited_count",                    self.uint8, 0, doc="Number of trigger decisions inhibited."),       s.field("new_td_inhibited_count",             self.uint8, 0, doc="Contributing trigger candidates associated with trigger decisions inhibited in the slice."),
       ...
       s.field("new_td_total_count",                    self.uint8, 0, doc="Number of trigger decisions created in the slice"),
       s.field("td_not_triggered_count",                self.uint8, 0, doc="Number of trigger decisions not passing triggerword check"),
       s.field("td_not_triggered_tc_count",             self.uint8, 0, doc="Number of contributing trigger candidates asssociated with decisions not passing triggerword check"),
       s.field("tc_data_vs_system_ms",                  self.uint8, 0, doc="Difference between data time and system time (ms)."),
       s.field("td_made_vs_ro_window_ms",                self.uint8, 0, doc="Difference between the time TD was made and the start of the readout window it is requesting (ms)."),
       s.field("td_send_vs_ro_start_ms",                  self.uint8, 0, doc="Difference between the time TD was made and the end of the readout window it is requesting (ms)."),
       s.field("td_send_vs_ro_end_ms",               self.uint8, 0, doc="Difference between the time TD was sent and the start of the readout window it is requesting (ms)."), 
